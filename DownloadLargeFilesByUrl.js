  ...
  DownloadLargeFilesByUrl = (function() {
    ...
    chkDl = function() {
      ...
      if (res.getResponseCode() === 206) {
        ...
        this.cfgVal.fileName = (headers["Content-Disposition"] && headers["Content-Disposition"].match(/filename=\"([a-zA-Z0-9\s\S].+)\";/)) ? headers["Content-Disposition"].match(/filename=\"([a-zA-Z0-9\s\S].+)\";/)[1].trim() : this.startTime.toString();
